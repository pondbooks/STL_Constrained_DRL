...
class Trainer:
    ...
    def evaluate(self, steps):
        ...
        for _ in range(self.num_eval_episodes): # 100
            evaluate_val = 1.0
            state = self.env_test.reset()
            eval_temp = self.env_test.evaluate_stl_formula() # Output 1.0
            ...
            while (not done):
                ...
                action = self.algo.exploit(state)
                state, reward, stl_reward, done, _ = self.env_test.step(action)
                eval_temp = self.env_test.evaluate_stl_formula() # evaluate the past trajectory: 0 is failed, 1 is success
                evaluate_val = min(evaluate_val, eval_temp) # \Phi = G\phi
