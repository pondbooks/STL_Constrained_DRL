...
class LagrangianSAC(trainer.Algorithm):
    ...
    def __init__(self, state_shape, action_shape, device=torch.device('cuda'), seed=0,
                batch_size=64, gamma=0.99, lr_actor=3e-4, lr_critic=3e-4, lr_entropy=3e-4, lr_kappa=1e-5, threshold=-40.,
                pretrain_steps=250000, replay_size=10**6, start_steps=10**4, tau=0.01, alpha=1.0, kappa=1.0, reward_scale=1.0, auto_coef=True):
        super().__init__()
    ...
    def update_pretrain_STL_critic(self, states, actions, stl_rewards, dones, next_states):
        ...
        with torch.no_grad():
            ...
            next_stl_qs = torch.min(next_stl_qs1, next_stl_qs2) - self.alpha * log_pis # Pretrain
    ...
    def update_finetune_STL_critic(self, states, actions, stl_rewards, dones, next_states):
        ...
        with torch.no_grad():
            ...
            next_stl_qs1, next_stl_qs2 = self.STL_critic_target(next_states, next_actions)
            next_stl_qs = torch.min(next_stl_qs1, next_stl_qs2) # Finetune
        target_stl_qs = stl_rewards * self.reward_scale + (1.0 - dones) * self.gamma * next_stl_qs
    ...
    def update_pretrain_actor(self, states):
        ...
        loss_actor = (self.alpha * log_pis - torch.min(STL_qs1, STL_qs2)).mean() # Pretrain
    ...
    def update_finetune_actor(self, states):
        ...
        STL_qs1, STL_qs2 = self.STL_critic(states, actions)
        loss_actor = (self.alpha * log_pis - torch.min(reward_qs1, reward_qs2) - self.kappa * (torch.min(STL_qs1, STL_qs2))).mean() # Finetune
