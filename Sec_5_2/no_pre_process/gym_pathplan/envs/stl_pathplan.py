...
# If you want to output gif animation, import the rendering from classical_control 
...
class STL_Problem_GF(gym.Env):
    ...
    def __init__(self):
        ...
        self.car_dim = 3
        #self.low_extended_space = np.array([-np.inf, -np.inf, -np.pi, -0.5 , -0.5], dtype=np.float32)# return obs dim (preprocessed)
        #self.high_extended_space =  np.array([np.inf, np.inf, np.pi, 0.5 , 0.5], dtype=np.float32)
        self.low_extended_space = -100*np.ones(self.tau*self.car_dim)
        self.high_extended_space =  100*np.ones(self.tau*self.car_dim)
    ...
    def reset(self): 
        ...
        self.observation = self.observe(self.past_tau_trajectory) # preprocessing
        
        self.done = False
        #self.success_val = 0.0 # GF
    ...
    def step(self, action): 
        ...
        self.past_tau_trajectory = self.past_tau_trajectory[1:] # update the extended state
        self.past_tau_trajectory.append(self.state.copy())

        #self.observation = self.preprocess(self.past_tau_trajectory) # preprocessing
        self.observation = self.observe(self.past_tau_trajectory) # no preprocessing
        
        self.num_steps += 1

        if self.num_steps == self._max_episode_steps: # 1000 steps
            return_done = True
            self.reset() 
        else:
            return_done = False
    ...
    #    obs = np.zeros(self.car_dim + 2) # dim(obs) = 5
    ...
    # nopreprocessing
    def observe(self, tau_state): 
        tau_num = len(tau_state)
        assert tau_num == self.tau, "dim of tau-state is wrong."

        # Define tau extended state
        obs = np.zeros(tau_num*self.car_dim) # 100*3 
        for i in range(tau_num):
    ...
    def STLreward(self, tau_state): 
        ...
        for i in range(tau_num):
            ...
            phi_1_rob = max(phi_1_rob, temp_1_rob) # F \varphi
            phi_2_rob = max(phi_2_rob, temp_2_rob) # F \varphi

        return_val = min(phi_1_rob, phi_2_rob) # land
    ...
    def evaluate_stl_formula(self): # called by trainer to evaluate satisfying the specification
        if self.num_steps >= self.tau-1: 
            ...
            returns = min(phi_1_rob, phi_2_rob) # land
    ...
    def better_region(self,state):
        ...
        robustness = min(robustness, psi3)
        robustness = min(robustness, psi4)
        robustness = min(robustness, 0.0) # nonpositive
        return robustness
    ...
    ''' def render(self, mode='human', close=False): # 環境を可視化する
        ...
        if self.viewer is None:
            ...
            # base
            b1_l, b1_r, b1_t, b1_b = rate_better_region_l*screen_width, rate_better_region_r*screen_width, rate_better_region_t*screen_height, rate_better_region_b*screen_height  
            ...
            # start
            start_l, start_r, start_t, start_b = rate_init_l*screen_width, rate_init_r*screen_width, rate_init_t*screen_height, rate_init_b*screen_height  
            ...
            # goal_1
            g1_l, g1_r, g1_t, g1_b = rate_stl_1_l*screen_width, rate_stl_1_r*screen_width, rate_stl_1_t*screen_height, rate_stl_1_b*screen_height 
            ...
            # goal_2
            g2_l, g2_r, g2_t, g2_b = rate_stl_2_l*screen_width, rate_stl_2_r*screen_width, rate_stl_2_t*screen_height, rate_stl_2_b*screen_height  
