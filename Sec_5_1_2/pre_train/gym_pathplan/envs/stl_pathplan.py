...
class STL_Problem_FG(gym.Env):
    ...
    def __init__(self):
        ...
        hrz_phi_2_ks = 0.
        hrz_phi_2_ke = 49.
        hrz_phi = max(hrz_phi_1_ke,hrz_phi_2_ke) # 49.
        self.phi_1_timebound = [hrz_phi_1_ks, hrz_phi_1_ke] # [0., 49.]
        self.phi_2_timebound = [hrz_phi_2_ks, hrz_phi_2_ke] # [0., 49.]
        self.tau = int(hrz_phi + 1) # 50
        self.tau_1 = hrz_phi_1_ke
        self.tau_2 = hrz_phi_2_ke
        ...
        self.safe_low_x = 0.5
        self.safe_low_y = 0.5
        self.safe_high_x = 4.5
        self.safe_high_y = 4.5
        # ##################
    ...
    def reset(self):
        ...
        self.observation = self.preprocess(self.past_tau_trajectory) # tau-MDP or F-MDP
    ...
    def preprocess(self, tau_state): # return pre-processed tau_mdp state
        ...
        for i in range(tau_num):
            if (i >= self.phi_1_timebound[0]) and (i < self.phi_1_timebound[1] + 1):
                if self.subSTL_1_robustness(tau_state[i]) >= 0:
                    f1 = min(f1 + 1/(float(self.phi_1_timebound[1] - self.phi_1_timebound[0] + 1)), 1.0)
                else:
                    f1 = 0.0
            if (i >= self.phi_2_timebound[0]) and (i < self.phi_2_timebound[1] + 1):
                if self.subSTL_2_robustness(tau_state[i]) >= 0:
                    f2 = min(f2 + 1/(float(self.phi_2_timebound[1] - self.phi_2_timebound[0] + 1)), 1.0)
                else:
                    f2 = 0.0
        obs[3] = f1 - 0.5
        obs[4] = f2 - 0.5
    ...
    def STLreward(self, tau_state): 
        ...
        for i in range(tau_num):
            if (i >= self.phi_1_timebound[0]) and (i < self.phi_1_timebound[1] + 1):
                ...
                phi_1_rob = min(phi_1_rob, temp_1_rob)
            if (i >= self.phi_2_timebound[0]) and (i < self.phi_2_timebound[1] + 1):
                temp_2_rob = self.subSTL_2_robustness(tau_state[i])
                phi_2_rob = min(phi_2_rob, temp_2_rob)

        return_val = max(phi_1_rob, phi_2_rob) # FG
        ...
        stl_reward = np.exp(self.beta*return_val)/np.exp(self.beta) # weighting 1/beta to prevent the value from becoming too large 
    ...
    def evaluate_stl_formula(self): # called by trainer to evaluate satisfying the specification
        if self.num_steps >= self.tau-1: 
            ...
            returns = max(phi_1_rob, phi_2_rob) 
    ...
    def better_region(self,state):
        psi1 = state[0] - self.safe_low_x # 0.5 < x < 4.5
        psi2 = self.safe_high_x - state[0]
        psi3 = state[1] - self.safe_low_y # 0.5 < y < 4.5
        psi4 = self.safe_high_y - state[1]
        robustness = min(psi1, psi2)
        robustness = min(robustness, psi3)
        robustness = min(robustness, psi4)
    ...
    ''' def render(self, mode='human', close=False): # 環境を可視化する
        ...
        rate_stl_3_l = self.safe_low_x / self._max_x_of_window
        rate_stl_3_r = self.safe_high_x  / self._max_x_of_window
        rate_stl_3_t = self.safe_high_y  / self._max_y_of_window
        rate_stl_3_b = self.safe_low_y / self._max_y_of_window

        if self.viewer is None:
            self.viewer = rendering.Viewer(screen_width, screen_height)

            # base
            b1_l, b1_r, b1_t, b1_b = rate_stl_3_l*screen_width, rate_stl_3_r*screen_width, rate_stl_3_t*screen_height, rate_stl_3_b*screen_height  # screen のサイズで与える．
